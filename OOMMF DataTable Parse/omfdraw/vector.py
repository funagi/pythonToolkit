...
class vector:
    ...
    def normalize(self):#单位化
        l = vector.length(self)
        return vector(self.x/l,self.y/l,self.z/l)
    def rotateZ(self, angle):#XY面内旋转
        angle = angle/180*math.pi
        return vector(self.x*math.cos(angle)-self.y*math.sin(angle),self.x*math.sin(angle)+self.y*math.cos(angle),self.z)
